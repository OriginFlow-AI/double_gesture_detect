...
int main(int argc, char** argv) {
    try {
        ...
        const auto config = std::filesystem::exists(args.config) ? double_ok_gesture::load_runtime_config(args.config)
                                                                 : double_ok_gesture::RuntimeConfig{};
        const auto csv = scan_feature_csv(args.csv);
        const auto model = summarize_file(args.model);
        if (!args.output.parent_path().empty()) {
            std::filesystem::create_directories(args.output.parent_path());
        }
        std::ofstream out(args.output);
        if (!out) {
            throw std::runtime_error("Cannot write report: " + args.output.string());
        }
        out << render_dashboard(args.config, config, csv, model);
        std::cout << args.output.string() << '\n';
        if (args.open) {
            maybe_open_browser(std::filesystem::absolute(args.output));
        }
        return 0;
